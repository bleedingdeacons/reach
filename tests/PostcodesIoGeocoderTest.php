...
function resultBody(float $lat, float $lng): string
{
 ...
 */
function placesBody(array $points): string
{
    $rows = array_map(
...
beforeEach(function () {
     ...
     */
    $this->stubByUrl = function (array $map, int &$calls = 0, ?string $default = null): void {
...
test('blank input returns null without touching the network', function () {
...
test('full postcode resolves via postcodes endpoint', function () {
    ($this->stubByUrl)([
        '/postcodes/BS14ST' => resultBody(51.4499, -2.5967),
    ]);
...
test('outcode resolves via outcodes endpoint', function () {
    ($this->stubByUrl)([
        '/outcodes/BS3' => resultBody(51.44, -2.60),
    ]);
...
test('place name resolves to first hit without bias', function () {
    ($this->stubByUrl)([
        '/places?q=Bedminster' => placesBody([
            [51.43, -2.60],
            [52.00, -1.00],
        ]),
    ]);
...
test('place name bias re ranks to nearest candidate', function () {
    ...
    ($this->stubByUrl)([
        '/outcodes/BS5'                 => resultBody(51.4636, -2.5520),
        '/places?q=Kingswood&limit=20'  => placesBody([
...
test('bias that does not geocode falls back to unbiased ranking', function () {
    // The bias outcode 404s, so getBias() yields null and the place
    // lookup reverts to first-hit ranking (and no &limit=20).
    ($this->stubByUrl)([
        '/outcodes/ZZ9'       => notFound(),
        '/places?q=Kingswood' => placesBody([[51.31, -0.20]]),
    ]);
...
test('successful lookup is cached and not refetched', function () {
    $calls = 0;
    ($this->stubByUrl)(
        ['/postcodes/BS14ST' => resultBody(51.45, -2.59)],
        $calls,
    );
...
test('miss is cached so repeated typos do not hammer the api', function () {
    $calls = 0;
    // Everything 404s / has no result, so the whole resolve chain misses.
    ($this->stubByUrl)([], $calls, notFound());
...
test('corrupt cache entry is discarded and re resolved', function () {
    ...
    ($this->stubByUrl)(['/postcodes/BS14ST' => resultBody(51.45, -2.59)]);
...
test('network error is swallowed and returns null', function () {
...
test('invalid json body returns null', function () {
    $this->stubHttp(static fn(string $url, array $args = [])
...
test('null coordinates in result are treated as a miss', function () {
...
test('full looking postcode falls through to outcode', function () {
    ...
    ($this->stubByUrl)([
        '/postcodes/BS3OO'    => notFound(),
        '/places?q=BS3OO'     => placesBody([[51.44, -2.60]]),
    ]);
