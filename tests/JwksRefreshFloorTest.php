...
{
    $header  = b64url((string) json_encode(['alg' => 'RS256', 'typ' => 'JWT', 'kid' => $kid]));
...
test('the floor is per key set not global', function () {
    ...
    $verifier = new JwtVerifier();
    $verifier->verify(tokenWithKid('x'), $this->jwksUrl, 'https://issuer.test', 'client-abc');
