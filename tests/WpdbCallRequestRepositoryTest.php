...
test('create stores only tracking data and returns model', function () {
...
test('list orders pending first then newest', function () {
...
test('count all returns var', function () {
    $db = new CallRequestWpdbStub();
    $db->nextVar = 12;
    $repo = new WpdbCallRequestRepository($db);
...
test('count pending filters on null completed at', function () {
    $db = new CallRequestWpdbStub();
    $db->nextVar = 4;
    $repo = new WpdbCallRequestRepository($db);
...
test('find by id hydrates completed row', function () {
    $db = new CallRequestWpdbStub();
    $db->nextRow = [
...
test('mark completed updates only pending row', function () {
    $db = new CallRequestWpdbStub();
    $db->queryReturn = 1;
    $repo = new WpdbCallRequestRepository($db);
...
test('mark completed returns false when nothing updated', function () {
    $db = new CallRequestWpdbStub();
    $db->queryReturn = 0;
    $repo = new WpdbCallRequestRepository($db);
...
test('delete returns true when row removed', function () {
    $db = new CallRequestWpdbStub();
    $db->deleteReturn = 1;
    $repo = new WpdbCallRequestRepository($db);
