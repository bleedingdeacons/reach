...
test('create rejects invalid attempt token', function () {
...
test('create rejects token minted for a different member', function () {
    ($this->seedSession)('viewer@example.com');
    $controller = ($this->makeController)();
...
test('create rejects token minted for a different viewer', function () {
    ($this->seedSession)('viewer@example.com');
    $controller = ($this->makeController)();
...
test('create records attempt and writes privacy preserving audit', function () {
...
test('audit falls back to unknown caller when viewer has no anonymous name', function () {
 ...
 */
test('rejects session whose member is no longer eligible', function () {
...
test('rejects write with another sessions token', function () {
    ($this->seedSession)('viewer@example.com');
    $repo = new SpyCallAttemptRepository();
    $controller = ($this->makeController)($repo);
