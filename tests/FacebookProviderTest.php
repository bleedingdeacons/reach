...
beforeEach(function () {
    ...
    $this->kid = 'fb-test-kid';

    $this->clientId = 'fb-app-id-123';

    $this->clientSecret = 'fb-app-secret';
     ...
     */
    $this->stubFacebookEndpoints = function (string $idToken, ?string &$capturedUrl = null, ?array &$capturedBody = null): void {
        $jwks = $this->jwks;
...
test('authorization url includes S256 challenge from verifier', function () {
...
test('authorization url requires verifier', function () {
    $provider = new FacebookProvider(($this->makeSettings)(), new JwtVerifier());
...
test('handle callback returns identity on happy path', function () {
    $provider = new FacebookProvider(($this->makeSettings)(), new JwtVerifier());
...
test('handle callback forwards verifier to token endpoint', function () {
    $provider = new FacebookProvider(($this->makeSettings)(), new JwtVerifier());
...
test('handle callback without verifier fails', function () {
    $provider = new FacebookProvider(($this->makeSettings)(), new JwtVerifier());
...
test('handle callback rejects unverified email', function () {
    $provider = new FacebookProvider(($this->makeSettings)(), new JwtVerifier());
...
test('handle callback rejects wrong issuer', function () {
    $provider = new FacebookProvider(($this->makeSettings)(), new JwtVerifier());
...
test('handle callback rejects wrong nonce', function () {
    $provider = new FacebookProvider(($this->makeSettings)(), new JwtVerifier());
...
test('handle callback returns identity for relay email', function () {
...
test('verify id token is not applicable', function () {
    $provider = new FacebookProvider(($this->makeSettings)(), new JwtVerifier());
    $this->expectException(\LogicException::class);
    $provider->verifyIdToken('whatever', 'nonce');
});

test('provider metadata', function () {
    $provider = new FacebookProvider(($this->makeSettings)(), new JwtVerifier());
