...
beforeEach(function () {
     ...
     */
    $this->storedCiphertext = function (string $sql): string {
...
test('install creates the table keyed by alert', function () {
...
test('save encrypts before writing', function () {
    $db = new WpdbStub();
...
test('save round trips through find', function () {
    $db = new WpdbStub();
    $repo = new WpdbAlertContactRepository($db);
...
test('find returns empty once the salt has rotated', function () {
...
test('find returns empty for tampered ciphertext', function () {
 ...
 */
dataset('absentColumns', function (): array {
    return [
        'no row'      => [null],
        'empty value' => [''],
        'not a string' => [0],
...
test('find returns empty when nothing is stored', function (mixed $stored) {
    $db = new WpdbStub();
    $db->nextVar = $stored;
...
test('save truncates an overlong contact', function () {
    $db = new WpdbStub();
    $repo = new WpdbAlertContactRepository($db);
...
test('save of a blank contact deletes instead', function () {
...
test('save reports failure when nothing was written', function () {
    $db = new WpdbStub();
...
test('has reports whether a row exists', function () {
    $db = new WpdbStub();
    $repo = new WpdbAlertContactRepository($db);
...
test('delete reports whether a row went away', function () {
    $db = new WpdbStub();
    $repo = new WpdbAlertContactRepository($db);
...
test('purge joins the alerts table to find expired contacts', function () {
